...
<template>
  ...
  >
    <div
      class="relative h-12 w-12 overflow-hidden rounded-full bg-black/5 md:h-14 md:w-14"
    >
      <img
        v-if="user?.avatarUrl"
        :src="user.avatarUrl"
    ...
    <div class="flex-1 text-center md:text-left">
      <p class="text-base font-medium text-[var(--bw-ink)] md:text-lg">
        Hello,
        <span class="font-semibold">{{ isPending ? '...' : user?.name }}</span>
      </p>
      <p class="text-[11px] text-[var(--bw-subtle)] md:text-xs">
        {{ dateLabel }}
      </p>
    </div>
