  ...
  <LayoutAppShell content-max-width="max-w-[860px]">
    <div class="space-y-5 pb-5 md:space-y-6">
      <div class="relative h-[220px] overflow-hidden rounded-[20px] sm:h-[240px] md:h-[280px]">
        <img
          :src="session?.imageUrl ?? 'https://www.figma.com/api/mcp/asset/22377b52-159d-4e36-8ade-0a08516b2b06'"
          :alt="session?.title ?? 'Class cover'"
          class="h-full w-full object-cover"
        />
        <NuxtLink to="/admin" class="absolute left-3 top-3 rounded-full bg-white/85 p-1.5">
          <Icon name="ph:arrow-circle-left" class="h-7 w-7 text-[var(--bw-ink)]" />
        </NuxtLink>
      </div>

      <div class="space-y-3">
        <h1 class="text-4xl font-semibold leading-none text-[var(--bw-ink)] sm:text-5xl">
        ...
        <div class="flex flex-wrap gap-2">
          ...
          <span class="inline-flex items-center gap-1 rounded-full bg-[#ececec] px-3 py-2 text-sm">
            ...
            <input v-model.number="slotsLeft" type="number" min="0" class="w-16 bg-transparent outline-none" /> slots left
          </span>
        </div>
      </div>
      ...
      <div>
        ...
        <div class="flex flex-wrap gap-2">
          ...
          >
            {{ tag }}
            <button type="button" class="text-[#1e8e3e]/70" @click="focusTags.splice(index, 1)">
      ...
      <div class="sticky bottom-[calc(env(safe-area-inset-bottom)+0.5rem)] grid gap-3 sm:grid-cols-2 md:static">
        <button class="rounded-[20px] border border-[#666] bg-white px-4 py-4 text-lg font-medium text-[#666] md:text-xl">
          Delete Session
        </button>
        <button class="rounded-[20px] bg-[var(--bw-orange)] px-4 py-4 text-lg font-semibold text-white md:text-xl">
          Update Session
        </button>
      </div>
    </div>
