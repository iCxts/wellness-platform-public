...
import { resolveAdminZoneId } from '~/services/admin'
...
const { mutateAsync, isPending } = useCreateAdminClass()
const errorMessage = ref('')

const levelMap: Record<
  string,
  'beginner' | 'pre_intermediate' | 'intermediate' | 'advanced'
> = {
  Beginner: 'beginner',
...
const focusMap: Record<
  ...
  | 'brain_refresh'
> = {
  'Neck & Shoulders': 'neck_shoulders',
  'Posture Reset': 'posture_reset',
  'Breathing Flow': 'breathing_flow',
  Core: 'core_strength',
  Mobility: 'hips_opener',
}

const toISODateTime = (dateISO: string, hhmm: string) =>
  new Date(`${dateISO}T${hhmm}:00`).toISOString()

const handleConfirm = async () => {
  errorMessage.value = ''

  if (!draft.value.instructorId) {
    errorMessage.value = 'Please choose an instructor first.'
    return
  }

  try {
    const resolvedZoneId = await resolveAdminZoneId({
      zoneId: draft.value.zoneId,
      zoneName: draft.value.zone || 'Wellness Center A',
    })
    draft.value.zoneId = resolvedZoneId

    await mutateAsync({
      title: draft.value.title,
      description: draft.value.description,
      level: levelMap[draft.value.intensity] ?? 'beginner',
      focus: draft.value.focus
        .map((item) => focusMap[item])
        .filter(
          (
            value,
          ): value is
            ...
            | 'posture_reset'
            | 'stress_release'
            | 'brain_refresh' => Boolean(value),
        ),
      roomName: draft.value.room,
      placeDescription: draft.value.zone,
      trainerId: draft.value.instructorId,
      zoneId: resolvedZoneId,
      startsAt: toISODateTime(draft.value.dateISO, draft.value.startTime),
      endsAt: toISODateTime(draft.value.dateISO, draft.value.endTime),
      capacity: draft.value.capacity,
    })
    await router.push('/admin/classes/create/success')
  } catch (error) {
    errorMessage.value =
      (error as { message?: string })?.message ?? 'Failed to create class.'
  }
}
</script>
      ...
      <div class="space-y-5 pb-5 md:space-y-6">
        ...
        <div class="grid gap-2 text-lg">
          <p class="flex items-center gap-2">
            <Icon name="ph:timer" class="h-6 w-6" /> APR 5 /
            {{ draft.startTime }} - {{ draft.endTime }} (45 min)
          </p>
          <p class="flex items-center gap-2">
            <Icon name="ph:map-pin-area" class="h-6 w-6" /> {{ draft.zone }} (
            {{ draft.room }} )
          </p>
        </div>
        ...
        <section
          class="grid gap-4 rounded-2xl border border-black/10 bg-white p-4 sm:grid-cols-[96px_1fr]"
        >
          <img
          ...
          <div class="space-y-1.5">
            <p class="text-4xl font-medium leading-none">
              {{ draft.instructorName }} <span class="text-xl">🇹🇭</span>
            </p>
        ...
        >
          <button
            ...
            @click="handleConfirm"
          >
            {{ isPending ? 'Creating...' : 'Confirm New Class!' }}
          </button>
        </div>
