...
const instructors = computed(() => instructorsQuery.data.value ?? [])
...
const selectInstructor = (instructor: { id: string; name: string }) => {
  draft.value.instructorId = instructor.id
  draft.value.instructorName = instructor.name
}

watchEffect(() => {
  if (draft.value.instructorId) return
  const firstInstructor = instructors.value[0]
  if (!firstInstructor) return
  draft.value.instructorId = firstInstructor.id
  draft.value.instructorName = firstInstructor.name
})
</script>
  ...
  <LayoutAppShell content-max-width="max-w-[900px]">
    <Motion v-bind="pageEnter" class="w-full min-w-0">
      <div class="space-y-5 pb-5 md:space-y-6">
        ...
        <section class="space-y-3">
          <p class="text-2xl font-bold">Instructor</p>
          <div class="grid gap-2 sm:grid-cols-2">
            <button
            ...
            >
              <p class="text-base font-semibold">{{ instructor.name }}</p>
              <p class="text-xs opacity-80">{{ instructor.email }}</p>
            </button>
          </div>
        </section>
        ...
        <section class="space-y-3">
          ...
          <div class="flex flex-wrap gap-2">
            <button
              ...
              :class="
                draft.focus.includes(item)
                  ? 'bg-[#e6f4ea] text-[#1e8e3e]'
                  : 'border border-[#cfd8d3] text-[#666]'
              "
        ...
        <section class="space-y-3">
          ...
          <div class="grid gap-3 sm:grid-cols-2">
            <button
              ...
              :class="
                draft.coverImageUrl === image
                  ? 'border-[var(--bw-orange)]'
                  : 'border-transparent'
              "
        ...
        <div
          class="bottom-[calc(env(safe-area-inset-bottom)+5rem)] flex justify-end pt-3 md:static"
        >
          <NuxtLink
